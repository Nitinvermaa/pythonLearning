#!/usr/bin/env python3
"""
Janus WebRTC Client using GStreamer
Streams camera feed to Janus server using WebRTC and SDP
"""
...
class JanusWebRTCClient:
    """Client for streaming camera feed to Janus server using GStreamer WebRTC"""
    
    def __init__(self, janus_url="http://localhost:8088/janus", room_id=1234, 
                 camera_device="/dev/video0", video_width=640, video_height=480, 
                 video_framerate=30):
        """
    ...
    async def join_room(self, display_name="Python Client"):
    ...
    async def _handle_offer(self, sdp_offer):
        ...
        if sdp_answer:
            ...
            answer_sdp = GstWebRTC.WEBRTCSDPType.ANSWER
    ...
    def on_ice_candidate(self, webrtcbin, mlineindex, candidate):
        ...
        if candidate:
            # Convert candidate to string if it's a GstStructure
            ...
            # Send ICE candidate to Janus via trickle ICE
            if self.loop:
                asyncio.run_coroutine_threadsafe(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    # Janus may not respond to trickle, that's OK
    ...
    def create_pipeline(self):
        ...
        # Create pipeline
        self.pipeline = Gst.Pipeline.new("webrtc-send")
        
        # Create elements
        src = Gst.ElementFactory.make("v4l2src", "src")
        src.set_property("device", self.camera_device)
        ...
        # Enable bundle for better compatibility
        self.webrtcbin.set_property("bundle-policy", 1)  # GstWebRTC.WebRTCBundlePolicy.MAX_COMPAT
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Janus WebRTC Camera Streaming Client")
